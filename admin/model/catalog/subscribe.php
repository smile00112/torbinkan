...
class ModelCatalogSubscribe extends Model {
	...
	public function addEmailDescription($descriptions) {
		...
		foreach ($descriptions as $language_id => $description) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "subscribe_email_description SET subscribe_descriptions = '" . $description . "', language_id = '" . (int) $language_id . "'");
		}
	}
	...
	public function addAuthDescription($descriptions) {
		...
		foreach ($descriptions as $language_id => $description) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "subscribe_auth_description SET subscribe_authorization = '" . $description . "', language_id = '" . (int) $language_id . "'");
		}
	}
	...
	public function editSubscribe($subscribe_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "subscribe SET email = '" . $data['email'] . "', status = '" . (int) $data['status'] . "' WHERE subscribe_id = '" . (int) $subscribe_id . "'");
	}
	...
	public function checkEmail($email, $subscribe_id) {
		$sql = "SELECT email FROM " . DB_PREFIX . "subscribe WHERE email='" . $email . "'";
