...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', mini_description = '" . $this->db->escape($value['mini_description']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', seo_title = '" . $this->db->escape($value['seo_title']) . "', seo_h1 = '" . $this->db->escape($value['seo_h1']) . "'");
		}
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					...
					if (isset($product_option['product_option_value']) && count($product_option['product_option_value']) > 0 ) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "' , unit_option = '" . $product_option_value['unit_option'] . "'  , amount_option = '" . $product_option_value['amount_option'] . "' , model_option = '" . $product_option_value['model_option'] . "'");
						} 
					}else{
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
					...
					'model_option'           => $product_option_value['model_option'],
					'unit_option'           => $product_option_value['unit_option'],
					'amount_option'           => $product_option_value['amount_option']
				);
			}
...
//
	public function getProductModelOption($product_id) {
		$query = $this->db->query("SELECT product_id, model_option FROM " . DB_PREFIX . "product_option_value WHERE product_id = '" . (int)$product_id . "'");
