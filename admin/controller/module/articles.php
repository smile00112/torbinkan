...
class ControllerModuleArticles extends Controller {
	...
	public function index() {
		...
		$this->model_catalog_articles->checkArticles();
	...
	public function insert() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			...
			$this->redirect($this->url->link('module/articles/listing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	private function getModule() { 
		...
		$this->data['entry_articlespage_thumb'] = $this->language->get('entry_articlespage_thumb');
		$this->data['entry_articlespage_popup'] = $this->language->get('entry_articlespage_popup');
		$this->data['entry_articlespage_addthis'] = $this->language->get('entry_articlespage_addthis');
		...
		if (isset($this->error['articlespage_thumb'])) {
			$this->data['error_articlespage_thumb'] = $this->error['articlespage_thumb'];
		} else {
			$this->data['error_articlespage_thumb'] = '';
		}
	
		if (isset($this->error['articlespage_popup'])) {
			$this->data['error_articlespage_popup'] = $this->error['articlespage_popup'];
		} else {
			$this->data['error_articlespage_popup'] = '';
		}
		...
		if (isset($this->request->post[$this->_name . '_articlespage_addthis'])) {
			$this->data[$this->_name . '_articlespage_addthis'] = $this->request->post[$this->_name . '_articlespage_addthis'];
		} else {
			$this->data[$this->_name . '_articlespage_addthis'] = $this->config->get($this->_name . '_articlespage_addthis');
		}
		...
		$this->template = 'module/articles.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/articles/listing', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);
	
		$this->data['module'] = $this->url->link('module/articles', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('module/articles/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('module/articles/delete', 'token=' . $this->session->data['token'], 'SSL');
	
		$this->data['totalarticles'] = $this->model_catalog_articles->getTotalArticles();
	
		$this->load->model('tool/image');
	
		$this->data['articles'] = array();
	
		$results = $this->model_catalog_articles->getArticles();
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/articles/update', 'token=' . $this->session->data['token'] . '&articles_id=' . $result['articles_id'], 'SSL')
			);
			...
			$this->data['articles'][] = array(
				'articles_id'     	=> $result['articles_id'],
				'title'       		=> $result['title'],
				'image'      		=> $image,
				'date_added'  	=> date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'viewed'			=> $result['viewed'],
				'status'     		=> ($result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')),
				'selected'    	=> isset($this->request->post['selected']) && in_array($result['articles_id'], $this->request->post['selected']),
				'action'      		=> $action
			);
		}
	
		$this->template = 'module/articles/list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() { 
		...
		if (!isset($this->request->get['articles_id'])) {
			$this->data['action'] = $this->url->link('module/articles/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/articles/update', 'token=' . $this->session->data['token'] . '&articles_id=' . $this->request->get['articles_id'], 'SSL');
		}
	
		$this->data['cancel'] = $this->url->link('module/articles/listing', 'token=' . $this->session->data['token'], 'SSL');
	
		if ((isset($this->request->get['articles_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$articles_info = $this->model_catalog_articles->getArticlesStory($this->request->get['articles_id']);
		}
		$this->load->model('localisation/language');
	
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
	
		if (isset($this->request->post['articles_description'])) {
			$this->data['articles_description'] = $this->request->post['articles_description'];
		} elseif (isset($this->request->get['articles_id'])) {
			$this->data['articles_description'] = $this->model_catalog_articles->getArticlesDescriptions($this->request->get['articles_id']);
		} else {
			$this->data['articles_description'] = array();
		}
		
		if (isset($this->request->post['meta_keyword'])) {
			$this->data['meta_keyword'] = $this->request->post['meta_keyword'];
		} elseif (isset($this->request->get['articles_id'])) {
			$this->data['meta_keyword'] = $this->model_catalog_articles->getArticlesDescriptions($this->request->get['articles_id']);
		} else {
		...
		} elseif (isset($this->request->get['date_added'])) {
			$this->data['date_added'] = date('Y-m-d', strtotime($this->request->get['date_added']));
		} else {
		...
		if (isset($this->request->post['articles_store'])) {
			$this->data['articles_store'] = $this->request->post['articles_store'];
		} elseif (isset($articles_info)) {
			$this->data['articles_store'] = $this->model_catalog_articles->getArticlesStores($this->request->get['articles_id']);
		} else {
			$this->data['articles_store'] = array(0);
		}
		...
		$this->template = 'module/articles/form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->request->post['articles_headline_chars']) {
			$this->error['numchars'] = $this->language->get('error_numchars');
		}
	
		if (!$this->request->post['articles_thumb_width'] || !$this->request->post['articles_thumb_height']) {
			$this->error['articlespage_thumb'] = $this->language->get('error_articlespage_thumb');
		}
	
		if (!$this->request->post['articles_popup_width'] || !$this->request->post['articles_popup_height']) {
			$this->error['articlespage_popup'] = $this->language->get('error_articlespage_popup');
		}
	...
	private function validateForm() {
		...
		foreach ($this->request->post['articles_description'] as $language_id => $value) {
	...
	public function install() { 
		...
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "articles`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "articles` (`articles_id` int(11) NOT NULL auto_increment, `status` int(1) NOT NULL default '0', `image` VARCHAR(255) COLLATE utf8_general_ci default NULL, `image_size` int(1) NOT NULL default '0', `date_added` date default NULL, `viewed` int(5) NOT NULL DEFAULT '0', PRIMARY KEY (`articles_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create articles description table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "articles_description`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "articles_description` (`articles_id` int(11) NOT NULL default '0', `language_id` int(11) NOT NULL default '0', `title` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_description` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, `description` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_keyword` varchar(255) COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`articles_id`,`language_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create articles store table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "articles_to_store`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "articles_to_store` (`articles_id` int(11) NOT NULL, `store_id` int(11) NOT NULL, PRIMARY KEY (`articles_id`, `store_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
    }
	...
	public function uninstall() { 
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE `query` LIKE 'articles_id=%'");
    }
}
?>