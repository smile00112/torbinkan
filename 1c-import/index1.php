	...
	//			print_r ($data[trim($theme['marking'])]);
				...
				if ($data[$theme['marking']])
				{
					...
					foreach ($data[$theme['marking']] as $item2)
					{
						...
						if ($item2['price'] > 0)
						{
							$array[] = array ('price' => (float)$item2['price'], 'marking' => $item2['art'], 'name' => $item2['name'], 'count' => $item2['count'], 'size' => $item2['name']);
							$array2[] = array ('Цена' => (float)$item2['price'], 'Артикул' => $item2['art'], 'Наименование товара' => $item2['name'], 'Единица измерения' => $item2['count'], 'Размер' => $item2['name']);
                         echo $item2['name'].' товар обновлен.<br>';   
                        }
					}
					if ($array)
					{
						//echo "UPDATE techcms_catalog_items SET multiprice = '".serialize ($array)."', multiprice_sys = '".serialize ($array2)."' WHERE id = '".$theme['id']."'";
...
<? } ?>
<div>
<a href="index.php">Первый вариант загрузки</a>
</div>