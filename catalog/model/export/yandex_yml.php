...
class ModelExportYandexYml extends Model {
	...
	public function getCategoryTree($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $allowed_manufacturers = '') {
		$this->loadCategories();
	...
	public function getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $allowed_manufacturers = '', $with_related = false, $skip = 0, $limit = 200) {
		$sql_blacklist = '';
		if ($blacklist) {
			$sql_blacklist = " AND ".($blacklist_type == 'black' ? "NOT" : "")."(p.product_id IN (" . $this->db->escape($blacklist) . "))";
		}
