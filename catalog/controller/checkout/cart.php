    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
        		...
        		foreach ($product['option'] as $option) {
					if ($option['type'] != 'file') {
				   	...
				   	$option_data[] = array(
                        ...
                        'model_option' => $option['model_option'],
                        'amount_option' => $option['amount_option'],
                        'unit_option' => $option['unit_option']
					);
        		}
				
				$sub_total = $this->cart->getOptTotal();
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				if ($sub_total >= 14999.00) {
				$price = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')));
				} else {			
				$price = $this->currency->format($this->tax->calculate($product['price_opt'], $product['tax_class_id'], $this->config->get('config_tax')));
				}
				...
				$total = $this->currency->format($this->tax->calculate($product['price_opt'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
				}
				} else {
					$total = false;
				}
